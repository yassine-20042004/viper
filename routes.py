...
def dashboard():
    ...
    # Vérifier les budgets pour les notifications
    data_manager.check_budget_notifications(session['user_id'])
    # Récupérer les notifications non lues
    notifications = data_manager.get_user_notifications(session['user_id'], unread_only=True)
...
def add_transaction():
    ...
    if request.method == 'POST':
        ...
        # Après avoir ajouté la transaction, vérifier les budgets
        data_manager.check_budget_notifications(session['user_id'])
...
def notifications():
    data_manager = DataManager()
    notifications = data_manager.get_user_notifications(session['user_id'])
...
def mark_notification_read(notification_id):
    data_manager = DataManager()
    if data_manager.mark_notification_as_read(notification_id):
        return jsonify({'success': True})
    return jsonify({'success': False}), 404


@app.route('/check-budgets')
@login_required
def check_budgets():
    data_manager = DataManager()
    data_manager.check_budget_notifications(session['user_id'])
